import { renderSchedule, fetchSchedule } from "../src/js/schedule";
...
describe("renderSchedule", () => {
  ...
  it('should render "No data found" when data is null', async () => {
    ...
    expect(table.innerHTML).toBe(
      '<tbody><tr><td colspan="4">No data found</td></tr></tbody>'
    );
  });
  ...
  it("should render schedule data correctly for multiple days and halls", async () => {
    ...
    const headerRow = table.querySelector(
      ".schedule-table__header th[colspan='3']"
    );
    ...
    expect(titleRow.querySelector("td:nth-child(2)").textContent).toBe(
      "Hall 1"
    );
    expect(titleRow.querySelector("td:nth-child(3)").textContent).toBe(
      "Hall 2"
    );
    expect(titleRow.querySelector("td:nth-child(4)").textContent).toBe(
      "Hall 3"
    );

    // Check class details in rows
    const firstClassRow = table.querySelector("tr:nth-child(4)");
    expect(firstClassRow.querySelector("td:nth-child(1)").textContent).toBe(
      "11:00"
    );
    expect(
      firstClassRow.querySelector("td:nth-child(2) .class-left p:nth-child(1)")
        .textContent
    ).toBe("Pilates");
  });
    ...
    rows.forEach((row, index) => {
      if (index > 1) {
        expect(
          row
            .querySelectorAll("td")
            .forEach((cell) => expect(cell.innerHTML).toBe(""))
        );
      }
    });
  });
